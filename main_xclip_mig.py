...
def init_distributed_mode():
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        # Get the actual number of available devices and verify
        device_count = torch.cuda.device_count()
        print(f"Actual available devices: {device_count}")
        
        # Safe device setting - ensure local_rank is within available device range
        if device_count > 0:
            # Use device 0 for all processes when MIG is enabled
            # This approach works with MIG because each process gets its own MIG slice
            device_id = 0
            torch.cuda.set_device(device_id)
            print(f"Set device to {device_id} for local_rank {local_rank}")
        else:
            print(f"WARNING: No CUDA devices found despite CUDA being available")
...
def set_seed_logger(args):
    ...
    # Get distributed information but don't set device here
    # We'll let init_device handle the device setting
    world_size = torch.distributed.get_world_size()
    args.world_size = world_size
    rank = torch.distributed.get_rank()
    args.rank = rank

    # DO NOT call torch.cuda.set_device here, it's handled in init_device
...
def init_device(args, local_rank):
    ...
    # When using MIG, we need to handle device assignment differently
    # Always use device 0 within each process when MIG is enabled
    device_id = 0
    torch.cuda.set_device(device_id)
    ...
    try:
        ...
        logger.info(f"Using device: {prop.name}, device_id: {device_id}")
    except RuntimeError as e:
        logger.error(f"Error accessing device {device_id}: {e}")
        logger.info("Falling back to CPU")
        device = torch.device("cpu")
    
    # In distributed training with MIG, each process uses one MIG instance
    # So we use n_gpu=1 for calculations
    n_gpu = 1
    ...
    if args.batch_size % args.n_gpu != 0 or args.batch_size_val % args.n_gpu != 0:
        raise ValueError(f"Invalid batch_size/batch_size_val and n_gpu parameter: {args.batch_size}%{args.n_gpu} and {args.batch_size_val}%{args.n_gpu}, should be == 0")
...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    # Create DDP model with current device only
    # When using MIG, always specify device_ids as [0]
    # This is because each process sees its own MIG partition as device 0
    model = torch.nn.parallel.DistributedDataParallel(
        model,
        device_ids=[0],  # Always use device 0 in MIG environment
        output_device=0,
...
def main():
    ...
    # IMPORTANT: Initialize distributed mode first
    local_rank = init_distributed_mode()
    args.local_rank = local_rank
    
    # IMPORTANT: Initialize device before seed_logger
    # This ensures device setup happens properly and avoids conflicts
    device, n_gpu = init_device(args, args.local_rank)
    
    # Now initialize seeds and logger
