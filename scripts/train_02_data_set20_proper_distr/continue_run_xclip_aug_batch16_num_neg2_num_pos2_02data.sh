...
job_name="xclip_vatex_aug_run_batch16_num_neg2_num_pos2_set20_proper_distribution"
...
python -m torch.distributed.run --nproc_per_node=1 main_xclip_aug.py \
    ...
    --test_path_from_data_folder splits_txt/vatex_val_avail_020.txt \
    --captions_path_from_data_folder splits_txt/captions_avail_formatted.json \
    --hard_positives_json_path ${DATA_PATH}/splits_txt/hard_positives_pos_20_proper_distribution.json \
    --hard_negatives_json_path ${DATA_PATH}/splits_txt/hard_positives_pos_20_proper_distribution.json \
    --use_wandb \
