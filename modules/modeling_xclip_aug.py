...
class XCLIP(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, word_ids_aug, word_token_type_ids_aug,
        ...
        # for tep_input_ids_aug, tep_token_type_ids_aug, tep_attention_mask_aug in zip(phrase_ids_aug,
        #                                                                              phrase_token_type_ids_aug,
        #                                                                              phrase_attention_mask_aug):
        #     tep_input_ids_aug = tep_input_ids_aug.squeeze(dim=0)
        #     tep_token_type_ids_aug = tep_token_type_ids_aug.squeeze(dim=0)
        #     tep_attention_mask_aug = tep_attention_mask_aug.squeeze(dim=0)
        #     tep_sequence_output_neg, tep_seq_features_neg = self.get_sequence_output(tep_input_ids_aug,
        ...
        if self.training:
            if self.loss_func == "fineGrained":
                # sequence_output_neg, seq_features_neg = self.get_sequence_output(word_ids_aug, word_token_type_ids_aug,
                #                                                                  word_attention_mask_aug,
                #                                                                  shaped=True)  # 1102
                loss = 0.
            ...
            if self.loss_func == "triplet":
                sequence_output_neg, seq_features_neg = self.get_sequence_output(word_ids_aug, word_token_type_ids_aug,
                                                                                 word_attention_mask_aug, shaped=True)
                loss = 0.
                sim_matrix, *_tmp = self.get_similarity_logits(sequence_output, seq_features, visual_output,
                                                               attention_mask,
                                                               video_mask, shaped=True, loose_type=self.loose_type)
                sim_loss1 = self.loss_fct(sim_matrix)
                sim_loss2 = self.loss_fct(sim_matrix.T)
                neg_sim_matrix, *_tmp = self.get_similarity_logits(sequence_output_neg, seq_features_neg, visual_output,
            ...
            if self.loss_func == "maxcol_v2t_phrase":
                ...
                for singel_video_feat, singel_video_mask, k_neg_sent_feat, k_neg_seq_features, k_neg_attention_mask in zip(
                        visual_output, video_mask, phrase_neg_sequence_output, phrase_neg_seq_features,
                        phrase_attention_mask_aug):
                    singel_video_feat = singel_video_feat.unsqueeze(dim=0)
                    singel_video_mask = singel_video_mask.unsqueeze(dim=0)
                    singe_v2t_sim_matrix_semantic, *_tmp = self.get_similarity_logits(k_neg_sent_feat,
            ...
            if self.loss_func == "maxcol_weight_coarse_word_phrase":
                ...
                v2negt_sim_matrix_phrase = torch.cat(v2negt_sim_matrix_phrase, dim=0)
                sim_loss_neg_phrase = self.loss_fct_col(v2negt_sim_matrix_phrase)
                sim_loss = self.coarse_weight * (
                            sim_loss1 + sim_loss2) / 2 + self.word_weight * sim_loss_neg + self.phrase_weight * sim_loss_neg_phrase
                loss += sim_loss
